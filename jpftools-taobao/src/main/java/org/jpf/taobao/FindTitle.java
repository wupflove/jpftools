...
public class FindTitle {
  ...
  public FindTitle() {
    ...
    try {
      ...
      JpfFileUtil.getFiles(strInputPath, vFile, ".xls");
      JpfFileUtil.getFiles(strInputPath, vFile, ".xlsx");
      for (int i = 0; i < vFile.size(); i++) {
        getTitles(vFile.get(i));
      }
      JpfFileUtil.saveFile("D:\\zgb\\标题汇总.csv", sb);
  ...
  private void getTitles(String strXlsFileName) {
    ...
    Workbook wb = null;
    try {
      if (!JpfFileUtil.FileExist(strXlsFileName)) {
        return;
      }
      ...
      while (row != null) {
        ...
        if (null != cell && cell.getStringCellValue() != null
          ...
          sb.append(strXlsFileName).append("\t").append(cell.getStringCellValue().trim())
              .append("\n");
