...
public class ShopManager extends Thread {
      ...
      for (int j = 0; j < vShopInfos.get(i).vGoodInfos.size(); j++) {
        ...
        try {
          ...
          strUrl = vShopInfos.get(i).shopUrl
              + "i/asynSearch.htm?_ksTS=1506296577909_120&callback=jsonp121&mid=w-16761742284-0&wid=16761742284&path=/search.htm&search=y&orderType=hotsell_desc&viewType=grid&keyword="
              + URLEncoder.encode(cGoodInfo.goodName.substring(cGoodInfo.goodName.length() - 3,
                  cGoodInfo.goodName.length()), "GB2312")
              + "&lowPrice=" + getNewPriceLow(cGoodInfo.goodPrice) + "&highPrice="
              + getNewPriceHigh(cGoodInfo.goodPrice);
          logger.info(strUrl);
        } catch (Exception ex) {
          // TODO: handle exception
    ...
    for (int i = 0; i < vShopInfos.size(); i++) {
      for (int j = 0; j < vShopInfos.get(i).vGoodInfos.size(); j++) {
        try {
          ...
          TitlesUtil.writeToCsv("店铺" + JpfDateTimeUtil.getCurrDate() + ".csv", sb);
          TextAreaLogAppender.log("输出文件:" + "店铺" + JpfDateTimeUtil.getCurrDate());
        } catch (Exception ex) {
          // TODO: handle exception
          ex.printStackTrace();
        }
  ...
  public static void main(String[] args) {
     ...
     * java.net.URLEncoder.encode("礼品盒", "GB2312"); System.out.println(mytext); //%C0%F1%C6%B7%BA%D0
     * } catch (Exception e) {
     * 
     * }
     */
